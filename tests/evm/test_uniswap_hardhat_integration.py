...
class TestUniswapV2HardhatIntegration:
    ...
    async def test_hardhat_connection(self, hardhat_blockchain):
        ...
        print(
            f"Connected to Hardhat. Latest block: {latest_block}, Accounts: {len(accounts)}"
        )
    ...
    async def test_quote_swap_integration(self, uniswap_v2, test_tokens):
        ...
        try:
            ...
            print(
                f"Quote: {quote.input_amount} {token_a.data.symbol} -> {quote.output_amount} {token_b.data.symbol}"
            )
    ...
    @pytest.mark.skip(reason="Requires actual pair creation and liquidity")
    async def test_build_swap_transaction_integration(
        self, uniswap_v2, test_tokens, test_wallet
    ):
        ...
        try:
            ...
            tx_params = await uniswap_v2.build_swap_transaction(
                route=quote, wallet=test_wallet
            )
    ...
    @pytest.mark.skip(
        reason="Requires actual pair creation and liquidity, and would execute real transaction"
    )
    async def test_create_swap_transaction_integration(
        self, uniswap_v2, test_tokens, test_wallet
    ):
    ...
    async def test_eth_to_token_swap_scenario(
        self, uniswap_v2, test_tokens, test_wallet, hardhat_blockchain
    ):
    ...
    async def test_quote_swap_v3_integration(self, uniswap_v3, test_tokens):
        ...
        try:
            ...
            print(
                f"V3 Quote: {quote.input_amount} {token_a.data.symbol} -> {quote.output_amount} {token_b.data.symbol}"
            )
    ...
    @pytest.mark.skip(reason="Requires actual V3 pool creation and liquidity")
    async def test_build_swap_transaction_v3_integration(
        self, uniswap_v3, test_tokens, test_wallet
    ):
        ...
        try:
            ...
            print("V3 Transaction built successfully:")
            print(f"  To: {tx_params['to']}")
            print(f"  Gas: {tx_params['gas']}")
            print(f"  From: {tx_params['from']}")
    ...
    async def test_v3_fee_tier_scenario(
        self, uniswap_v3, test_tokens, test_wallet, hardhat_blockchain
    ):
        ...
        print(
            "V3 multi-fee-tier scenario structure verified (would require liquidity to execute)"
        )
...
class TestUniswapV3ErrorHandling:
    ...
    async def test_multi_hop_not_implemented_v3(
        self, uniswap_v3, test_tokens, test_wallet
    ):
        ...
        with pytest.raises(ValueError, match="Multi-hop swaps not implemented"):
            await uniswap_v3.build_swap_transaction(
                route=mock_route, wallet=test_wallet
            )
