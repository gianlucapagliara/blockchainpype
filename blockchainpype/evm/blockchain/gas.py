...
class GasConfiguration(BaseModel):
    ...
    async def estimate_eip1559_gas_fees(
        ...
        Args:
            w3 (AsyncWeb3[Any]): Web3 instance for blockchain interaction
            transaction_params (TxParams | None): Optional transaction parameters
            n_max_retries (int): Maximum number of estimation attempts before
        ...
        Raises:
            Exception: The last estimation error, if all attempts fail
        """
        retries = 0
        ...
        while True:
            try:
                ...
                # Worst-case bound: the base fee can grow at most 12.5% per
                # block, so doubling covers several blocks of inclusion delay.
    ...
    async def estimate_legacy_gas_fees(
        ...
        Raises:
            KeyError: If the configured gas mode is unknown
        """
