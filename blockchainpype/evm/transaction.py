...
class EthereumTransactionReceipt(BlockchainTransactionReceipt):
    ...
    Attributes:
        ...
        contract_address (EthereumAddress | None): Address of the created contract,
            None for all non-contract-creation transactions
        ...
        root (HexStr | None): Post-transaction state root, only present on
            pre-Byzantium receipts
        status (int | None): Transaction status (1 for success, 0 for failure),
            None on pre-Byzantium receipts which carry the state root instead
        to (EthereumAddress | None): Recipient address, None for contract creation
    ...
    def from_raw(cls, raw_transaction_receipt: TxReceipt) -> Self:
        ...
        Handles the shapes real chains return: contractAddress is None for
        non-contract-creation transactions, 'to' is None for contract creation,
        and post-Byzantium receipts carry 'status' instead of 'root'.
        ...
        """
        contract_address = raw_transaction_receipt.get("contractAddress")
        to_address = raw_transaction_receipt.get("to")

        return cls(
            block_hash=EthereumBlockHash.from_raw(raw_transaction_receipt["blockHash"]),
            block_number=raw_transaction_receipt["blockNumber"],
            contract_address=EthereumAddress.from_raw(contract_address)
            ...
            logs=list(raw_transaction_receipt["logs"]),
            logs_bloom=raw_transaction_receipt["logsBloom"],
            root=raw_transaction_receipt.get("root"),
            status=raw_transaction_receipt.get("status"),
            to=EthereumAddress.from_raw(to_address) if to_address is not None else None,
            ...
            type=raw_transaction_receipt.get("type", 0),
        )
...
class EthereumRawTransaction(BaseModel):
    ...
    Attributes:
        ...
        block_number (BlockNumber | None): Block number where this transaction was
            included, None while the transaction is pending
    ...
    def from_raw(cls, raw_transaction: TxData) -> Self:
        ...
        Handles the shapes real chains return: pending transactions have None
        block fields, contract creations have a None 'to', and legacy
        transactions carry 'gasPrice' instead of the EIP-1559 fee fields.
        ...
        """
        raw_sender = raw_transaction.get("from")
        raw_to = raw_transaction.get("to")
        ...
        kwargs: dict[str, Any] = {
            ...
            "sender": EthereumAddress.from_raw(raw_sender)
            if raw_sender is not None
            else None,
            "gas": raw_transaction.get("gas", None),
            "gas_price": raw_transaction.get("gasPrice", None),
            ...
            "to": EthereumAddress.from_raw(raw_to) if raw_to is not None else None,
        ...
        # Optional fields default to None; required fields (sender, nonce)
        # missing from the payload surface as a pydantic ValidationError.
